...
impl CipherHud {
    pub fn new(cc: &eframe::CreationContext<'_>, rx: Receiver<TelemetryUpdate>, tx_user: tokio::sync::mpsc::UnboundedSender<String>) -> Self {
        // Customize looks
...
impl eframe::App for CipherHud {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        while let Ok(update) = self.rx.try_recv() {
            if let Some(e) = update.epistemic { self.epistemic = e; }
            if let Some(e) = update.entropy { self.entropy = e; }
            if let Some(s) = update.social { self.social = s; }
            if let Some(uptime) = update.uptime_secs { self.uptime_secs = uptime; }
            if let Some(skills) = update.active_skills { self.active_skills = skills; }
            
            if let Some(tu) = update.token_usage { self.token_usage = tu; }
            if let Some(cf) = update.context_fullness { self.context_fullness = cf; }
            if let Some(subj) = update.learning_subject { self.learning_subject = subj; }
            if let Some(tb) = update.treasury_balances { self.treasury_balances = tb; }
            if let Some(ss) = update.socialization_status { self.socialization_status = ss; }

            if let Some(action) = update.verified_action {
                self.verified_actions.push(action);
                if self.verified_actions.len() > 50 { self.verified_actions.remove(0); }
            }
            if let Some(task) = update.follow_up_task {
                self.follow_up_tasks.push(task);
                if self.follow_up_tasks.len() > 50 { self.follow_up_tasks.remove(0); }
            }
        ...
        egui::SidePanel::left("left_panel")
            ...
            .show(ctx, |ui| {
                ...
                ui.add(egui::ProgressBar::new(self.epistemic as f32).animate(true).text(format!("{:.2}", self.epistemic)));

                ui.add_space(5.0);
                ui.label("ENTROPY (Order/Chaos)");
                ui.add(egui::ProgressBar::new(self.entropy as f32).animate(true).text(format!("{:.2}", self.entropy)).fill(egui::Color32::from_rgb(200, 50, 50)));

                ui.add_space(5.0);
                ui.label("SOCIAL (Isolation)");
                ui.add(egui::ProgressBar::new(self.social as f32).animate(true).text(format!("{:.2}", self.social)));
                ...
                ui.add(egui::ProgressBar::new(self.context_fullness).animate(true).text(format!("{:.1}%", self.context_fullness * 100.0)).fill(egui::Color32::from_rgb(100, 150, 255)));
            });

        // --- RIGHT PANEL (CAPITAL & RESEARCH) ---
        egui::SidePanel::right("right_panel")
            ...
            .show(ctx, |ui| {
                ...
                ui.label(egui::RichText::new(&self.learning_subject).color(egui::Color32::from_rgb(0, 200, 255)));
                ...
                ui.add_space(5.0);
                ui.label("STATUS:");
                ui.label(egui::RichText::new(&self.socialization_status).color(egui::Color32::LIGHT_BLUE));
            });

        // --- CENTRAL PANEL (SEMANTIC OODA LOOP) ---
        egui::CentralPanel::default().show(ctx, |ui| {
            // Split Central Panel internally into top and bottom (or side by side)
            ui.columns(2, |columns| {
                // Column 0: Verified Activity
                columns[0].label(egui::RichText::new("✅ VERIFIED ACTIONS").strong().color(egui::Color32::GREEN));
                columns[0].separator();
                egui::ScrollArea::vertical().id_source("verified_scroll").auto_shrink([false, false]).stick_to_bottom(true).show(&mut columns[0], |ui| {
                    for action in &self.verified_actions {
                        ui.label(egui::RichText::new(format!("• {}", action)).color(egui::Color32::WHITE));
                        ui.add_space(2.0);
                    }
                });

                // Column 1: Follow-Up Trajectory
                columns[1].label(egui::RichText::new("🎯 DIRECTIVES & TRAJECTORY").strong().color(egui::Color32::from_rgb(255, 140, 0)));
                columns[1].separator();
                egui::ScrollArea::vertical().id_source("trajectory_scroll").auto_shrink([false, false]).stick_to_bottom(true).show(&mut columns[1], |ui| {
                    for task in &self.follow_up_tasks {
                        ui.label(egui::RichText::new(format!("→ {}", task)).color(egui::Color32::LIGHT_GRAY));
                        ui.add_space(2.0);
                    }
                });
            });
            ...
            ui.add_space(10.0);
            ui.separator();
            ui.label(egui::RichText::new("RAW SYSTEM FRAGMENT LOGS:").small().color(egui::Color32::DARK_GRAY));
            egui::ScrollArea::vertical().id_source("log_scroll").max_height(100.0).auto_shrink([false, false]).stick_to_bottom(true).show(ui, |ui| {
                for log in &self.logs {
                    ui.label(egui::RichText::new(log).color(egui::Color32::from_rgb(80, 80, 80)).small());
                }
            });
        });
